...
- **lounge/[id].tsx** (1154) — realtime chat. All writes via `useLoungeStore` (sendMessage sanitizes per stores audit); subscription lifecycle correct (`subscribeToLounge`→unsub + markRead on unmount, `:411`); member list is a read with defensive profiles-shape handling; **TYPES-1 ordering fix confirmed in place** (`:428-429` comment: chronological top→bottom FlashList, no manual reverse). Minor: member-fetch `.then()` (`:398`) lacks an unmount guard (benign setState-after-unmount at worst). Clean.
