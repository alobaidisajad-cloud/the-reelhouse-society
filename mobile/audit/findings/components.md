...
**Surfaces confirmed unguarded (all render review text without spoiler handling):** feed `ReviewContent`, log-detail `LogReviewBody`, home `PulseCardItem`, **film-detail `FilmReviews`** (its `CommunityReview` type doesn't even include `is_spoiler`), and `film-reviews/[id]` "read all". So a spoiler-flagged review is exposed on every review-display surface in the app.
...
**✅ CONFIRMED with line-level proof (this pass):** `src/services/ProfileWriteService.ts:52` (`dbUpdates.preferences = updates.preferences`) → `:67` (`supabase.from('profiles').update(validatedData).eq('id', userId)`) is a full-column overwrite. In `ProgrammesSection.tsx:127` the `updateUser({preferences})` call is fired **un-awaited** (full-blob write) and `:136` awaits the `update_my_preferences` merge RPC — the two DB writes **race**; if the blob write lands last it clobbers a concurrent cross-device change to any *other* preference key. (Note: ProgrammesSection is otherwise hardened — reads fresh prefs from the store, rolls back on hard failure, enqueues on network error.) Severity stays MEDIUM. Fix is unchanged: drop `preferences` from `ProfileWriteService.updateProfile` so ALL preference writes go through the merge RPC / `auth.setPreference`. The full-blob overwrite bypasses the server-side JSONB merge that `auth.setPreference` uses, so a concurrent cross-device change to a *different* preference key can be clobbered. Root cause: two preference-write paths exist — `auth.setPreference` (RPC merge, cross-device-safe) vs `auth.updateUser`/`ProfileService` (full-column overwrite). `SettingsScreen` also writes preferences via the overwrite path.
