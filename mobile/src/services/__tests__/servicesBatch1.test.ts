...
describe('NotificationService', () => {
    ...
    describe('getNotifications', () => {
        it('fetches notifications for user', async () => {
            const notifs = [{ id: 'n1', type: 'endorse', message: 'Liked your log', read: false }];
            const c = chain({ data: notifs, error: null });
            (supabase.from as jest.Mock).mockReturnValue(c);
    ...
    describe('markAllAsRead', () => {
        it('updates unread notifications for user', async () => {
            const c = chain({ error: null });
            (supabase.from as jest.Mock).mockReturnValue(c);
    ...
    describe('markAsRead', () => {
        it('updates single notification', async () => {
            ...
            expect(c.update).toHaveBeenCalledWith({ read: true });
            expect(c.eq).toHaveBeenCalledWith('id', 'n1');
        });

        it('throws on error', async () => {
            const c = chain({ error: { message: 'DB error' } });
            (supabase.from as jest.Mock).mockReturnValue(c);
            await expect(NotificationService.markAsRead('n1')).rejects.toBeTruthy();
        });
    });
});
