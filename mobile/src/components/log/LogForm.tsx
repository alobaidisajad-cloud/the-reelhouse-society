...
import { PHYSICAL_OPTIONS, RATING_LABELS, ABANDONED_REASONS } from '@/src/hooks/useLogFlow';
        ...
        <View style={{ flex: 1 }}>
                ...
                <Animated.View entering={FadeInDown.duration(200)}>
                    ...
                    <View style={st.sec}>
                        ...
                        {watchedWith.includes('@') && user?.following && user.following.length > 0 && (() => {
                            ...
                            const matches = user.following.filter((u: string) => u.toLowerCase().startsWith(partial)).slice(0, 4);
                    ...
                    <View style={st.sec}>
                        ...
                        <TextInput style={st.reviewInput} placeholder="Write your thoughts as if typing on a manuscript..." placeholderTextColor={colors.fog} value={review} onChangeText={setReview} multiline maxLength={2000} textAlignVertical="top" selectionColor={'rgba(218,165,32,0.3)'} cursorColor={colors.sepia} disableFullscreenUI={true} keyboardAppearance="dark" accessibilityLabel="Write your film review" />
                    ...
                    <AuteurToolkit
                        ...
                        onUpgradePress={() => router.push('/membership' as any)}
                    />
                    ...
                    <View style={st.sec}>
                        ...
                        {!isPremium && (
                            <PressableScale style={st.upgradeRow} onPress={() => router.push('/membership' as any)} hitSlop={{ top: 15, bottom: 15, left: 15, right: 15 }} haptic="light">
                    ...
                    <View style={st.sec}>
                        ...
                        {isPremium ? (
                            ...
                            <View style={st.lockedBox}>
                                ...
                                <PressableScale onPress={() => { router.push('/membership' as any); }} hitSlop={{top: 10, bottom: 10, left: 20, right: 20}} haptic="light">
            ...
            {lists.length > 0 && (
                <View style={st.sec}>
                    ...
                    <FlashList horizontal data={lists} showsHorizontalScrollIndicator={false} keyExtractor={(l: any) => l.id} contentContainerStyle={st.flatListGapPad}
                        estimatedItemSize={100}
                        ListFooterComponent={<View style={{ width: 20 }} />}
                        renderItem={({ item: list }: any) => {
                            const isIn = list.films.some((f: any) => f.id === film.id);
                            return (
                                <PressableScale style={[st.listChip, isIn && st.listChipOn]} onPress={() => { toggleList(list.id); }} haptic="selection">
            ...
            <View style={st.submitRow}>
                ...
                <PressableScale style={st.cancelBtn} onPress={() => { router.back(); }} hitSlop={{top: 15, bottom: 15, left: 15, right: 15}} haptic="selection">
