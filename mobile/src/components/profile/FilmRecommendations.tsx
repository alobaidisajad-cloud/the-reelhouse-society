...
export function FilmRecommendations({ logs }: FilmRecommendationsProps) {
    ...
    const [recs, setRecs] = useState<{ id: number; title: string; poster_path?: string | null; vote_average?: number | null }[]>([]);
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const allSimilar: { id: number; title: string; poster_path?: string | null; vote_average?: number }[] = [];
            ...
            for (const log of sample) {
                ...
                if (detail?.similar?.results) {
                    allSimilar.push(...(detail.similar.results as any[]));
                }
            }
