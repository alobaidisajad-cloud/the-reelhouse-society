...
function CardContent({ film, log, posterDataUrl, blurDataUrl, username }: { film: any, log: any, posterDataUrl: string | null, blurDataUrl: string | null, username?: string | null }) {
...
export default function DossierExportModal({
    ...
    const preGenerate = useCallback(async () => {
        try {
            const sharpUrl = getProxiedImageUrl(film.poster_path)
            const blurredUrl = getBlurredProxiedImageUrl(film.poster_path)
            ...
            try {
                ...
                const directUrl = tmdb.poster(film.poster_path, 'w500')
                if (directUrl) {
                    try {
            ...
            if (blurredUrl) {
                try {
                    ...
                    const directUrl = tmdb.poster(film.poster_path, 'w92')
                    if (directUrl) {
                        try {
            ...
            console.error('[DossierExport] pre-gen failed', err)
            setReady(true)
        }
    }, [film.poster_path])
        ...
        <Portal>
            <AnimatePresence>
                ...
                >
                    ...
                    >
                        <CardContent film={film} log={log} posterDataUrl={posterDataUrl} blurDataUrl={blurDataUrl} username={username} />
                    </div>
