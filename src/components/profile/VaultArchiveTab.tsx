...
export function VaultArchiveTab({ profileLogs, isOwnProfile, setViewLog, archiveSieve, setArchiveSieve, archiveVisibleCount, setArchiveVisibleCount, archiveFilteredLogs }: VaultArchiveTabProps) {
...
export function VaultWatchlistTab({ profileWatchlist, isOwnProfile }: { profileWatchlist: WatchlistItem[], isOwnProfile: boolean }) {
    const { isTouch: IS_TOUCH } = useViewport()
    return (
        <div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: IS_TOUCH ? '0.2rem' : '1rem' }}>
                        {profileWatchlist.map((film) => (
                            <Link key={film.id} to={`/film/${film.id}`}>
                                <motion.div whileHover={{ y: -3, transition: { type: 'spring', damping: 12 } }}><FilmCard film={film as any} /></motion.div>
                            </Link>
                        ))}
                    </div>
                </>
